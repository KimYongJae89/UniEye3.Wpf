...
{
    public class ThicknessDataImporter
    {
        //DB Info
        ...
        public int ImportMaxRowIndex(string lotName)
        {
            ...
            string fromQuery = $"from (select row_number() over() as row from \"TM_Traverse\" where lot_name = '{lotName}') t";
        ...
        public List<Dictionary<string, object>> ImportThicknessData(string lotName, int frameIndex)
        {
            ...
            string fullQuery = $"select * from \"TM_Data\" {whereQuery} order by traverse_position";
