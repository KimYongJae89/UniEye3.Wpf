...
using DeviceManager = WPF.UniScanIM.Override.DeviceManager;
using DeviceMonitor = WPF.UniScanIM.Override.DeviceMonitor;
using SystemManager = WPF.UniScanIM.Override.SystemManager;

namespace WPF.UniScanIM
{
    /// <summary>
    ...
    public partial class App : Application
    {
        ...
        protected override async void OnStartup(StartupEventArgs e)
        {
            ...
            try
            {
                var mutex = new Mutex(true, "WPF.UniScanIM", out bool bNew);
                if (!bNew)
                {
                ...
                ThreadPool.SetMinThreads(256, completionPortThreads);
        ...
        private void BuildSystemManager()
        {
            ...
            SystemManager.SetInstance(new SystemManager());

            AlgorithmFactory.SetInstance(new AlgorithmFactory());
            InspectRunner inspectRunner = new IMInspectRunner { InspectEventHandler = new InspectEventHandler() };
            SystemManager.Instance().AddInspectRunner(inspectRunner);

            ModelManager.SetInstance(new ModelManager(new ModelBuilder()));
