...
namespace DynMvp.Devices.FrameGrabber
{
    ...
    {
        #region 필드
        private const int bufferPoolCount = 5;
        ...
        {
            var cameraInfoMil = (CameraInfoMil)cameraInfo;
            ...
            string dcfFileName = cameraInfoMil.DcfFilePath;
            if (string.IsNullOrEmpty(dcfFileName))
            {
                dcfFileName = GetDcfFile(cameraInfoMil.CameraType);
            }
            ...
            if (NumOfBand == 1)
            {
                ...
                {
                    GrabImageBuffer[i] = MIL.MbufAlloc2d(cameraInfoMil.MilSystem.SystemId, width, height, MIL.M_UNSIGNED + 8, MIL.M_IMAGE + MIL.M_PROC + MIL.M_GRAB, MIL.M_NULL);
                }
            }
            else
            {
                for (int i = 0; i < GrabImageBuffer.Length; i++)
                {
                    GrabImageBuffer[i] = MIL.MbufAllocColor(cameraInfoMil.MilSystem.SystemId, 3, width, height, MIL.M_UNSIGNED + 8, MIL.M_IMAGE + MIL.M_PROC + MIL.M_GRAB, MIL.M_NULL);
                }
            }
        ...
        private string GetDcfFile(CameraType cameraType)
        {
            switch (cameraType)
            {
                ...
                case CameraType.VH_5MC_C16_2Tap:
                    return "VH-5MC-C16_2Tap_Periodic.dcf";
                case CameraType.RaL12288_66km:
                    return "Basler_Ace RAL12288-66km.dcf";
            }
        ...
        private static MIL_INT ProcessingFunction(MIL_INT HookType, MIL_ID HookId, IntPtr HookDataPtr)
        {
            ...
            var cameraMil = hUserData.Target as CameraMil;
            cameraMil.GrabbedImageQ.Enqueue(currentImageId);
            LogHelper.Debug(LoggerType.Grab, $"CameraMil::ProcessingFunction - Name: {cameraMil.Name}, grabbedImageQ: {cameraMil.GrabbedImageQ.Count}");
