...
namespace DynMvp.Devices.FrameGrabber
{
    ...
    public class ImageProvider
    {
        ...
        protected class GrabResult
        {
            ...
            public PylonC.NET.PYLON_STREAMBUFFER_HANDLE Handle; /* Holds the handle of the image registered at the stream grabber. It is used to queue the buffer associated with itself for the next grab. */
        }
        ...
        protected PylonC.NET.PYLON_IMAGE_FORMAT_CONVERTER_HANDLE m_hConverter; /* The format converter is used mainly for coverting color images. It is not used for Mono8 or RGBA8packed images. */
        protected Dictionary<PylonC.NET.PYLON_STREAMBUFFER_HANDLE, PylonC.NET.PylonBuffer<Byte>> m_convertedBuffers; /* Holds handles and buffers used for converted images. It is not used for Mono8 or RGBA8packed images.*/
        protected PylonC.NET.PYLON_DEVICE_HANDLE m_hDevice;           /* Handle for the pylon device. */
        public PylonC.NET.PYLON_DEVICE_HANDLE DeviceHandle => m_hDevice;

        protected PylonC.NET.PYLON_STREAMGRABBER_HANDLE m_hGrabber;   /* Handle for the pylon stream grabber. */
        protected PylonC.NET.PYLON_DEVICECALLBACK_HANDLE m_hRemovalCallback;    /* Required for deregistering the callback. */
        protected PylonC.NET.PYLON_WAITOBJECT_HANDLE m_hWait;         /* Handle used for waiting for a grab to be finished. */
        protected PylonC.NET.PYLON_WAITOBJECT_HANDLE m_hStop;         /* Handle used for waiting for a grab to be finished. */
        ...
        protected Dictionary<PylonC.NET.PYLON_STREAMBUFFER_HANDLE, PylonC.NET.PylonBuffer<Byte>> m_buffers; /* Holds handles and buffers used for grabbing. */
        protected List<GrabResult> m_grabbedBuffers; /* List of grab results already grabbed. */
        protected PylonC.NET.DeviceCallbackHandler m_callbackHandler; /* Handles callbacks from a device .*/
        ...
        {
            string lastErrorMessage = PylonC.NET.GenApi.GetLastErrorMessage();
            string lastErrorDetail = PylonC.NET.GenApi.GetLastErrorDetail();
        ...
        public ImageProvider()
        {
            ...
            m_buffers = new Dictionary<PylonC.NET.PYLON_STREAMBUFFER_HANDLE, PylonC.NET.PylonBuffer<Byte>>();
            m_grabbedBuffers = new List<GrabResult>();
            /* Create handles. */
            m_hGrabber = new PylonC.NET.PYLON_STREAMGRABBER_HANDLE();
            m_hDevice = new PylonC.NET.PYLON_DEVICE_HANDLE();
            m_hRemovalCallback = new PylonC.NET.PYLON_DEVICECALLBACK_HANDLE();
            m_hConverter = new PylonC.NET.PYLON_IMAGE_FORMAT_CONVERTER_HANDLE();
            /* Create callback handler and attach the method. */
            m_callbackHandler = new PylonC.NET.DeviceCallbackHandler();
            m_callbackHandler.CallbackEvent += new PylonC.NET.DeviceCallbackHandler.DeviceCallback(RemovalCallbackHandler);
        ...
        public PylonC.NET.NODE_HANDLE GetNodeFromDevice(string name)
        {
            if (m_open && !m_removed)
            {
                NODEMAP_HANDLE hNodemap = PylonC.NET.Pylon.DeviceGetNodeMap(m_hDevice);
                return PylonC.NET.GenApi.NodeMapGetNode(hNodemap, name);
            }
            return new PylonC.NET.NODE_HANDLE();
        }

        /* Open using device.*/
        protected void Open(PylonC.NET.PYLON_DEVICE_HANDLE device)
        {
            try
            {
                /* Use provided device. */
                ...
                if (PylonC.NET.Pylon.DeviceFeatureIsWritable(m_hDevice, "GevSCPSPacketSize"))
                {
                    ...
                    PylonC.NET.Pylon.DeviceSetIntegerFeature(m_hDevice, "GevSCPSPacketSize", 8192);
                }
        ...
        protected void SetupGrab()
        {
            ...
            {
                var buffer = new PylonC.NET.PylonBuffer<byte>(payloadSize, true);
        ...
        protected async void Grab()
        {
            try
            {
                ...
                while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                {
                    ...
                    uint timeoutMs = (uint)DeviceConfig.Instance().GrabTimeoutMs;
                    if (timeoutMs == 0)
                    {
                        timeoutMs = uint.MaxValue;
                    }

                    uint index = await WaitObjectsWaitForAny(waitObjects, timeoutMs);
                    //if (!PylonC.NET.Pylon.WaitObjectsWaitForAny(waitObjects, timeoutMs, out index))
                    if (index == 1)
                    {
                        lock (m_lockObject)
                        {
                            if (m_grabbedBuffers.Count != m_numberOfBuffersUsed)
                            {
                                ...
                                //System.Console.WriteLine("A grab timeout occurred.");

                                Debug.WriteLine("Pylon - Grab Timeout", DateTime.Now.ToString("HHmmss"));
                    ...
                    if (!PylonC.NET.Pylon.StreamGrabberRetrieveResult(m_hGrabber, out PylonGrabResult_t grabResult))
                    {
                    ...
                    if (grabResult.Status == PylonC.NET.EPylonGrabStatus.Grabbed)
                    {
                    ...
                    else if (grabResult.Status == PylonC.NET.EPylonGrabStatus.Failed)
                    {
                        /*
                        ...
                        */
                        ...
                        // 단발성으로 Grab이 실패할 경우에도 이 구문으로 들어온다.
                        // 임시로 그냥 진행하도록 변경
                        System.Console.WriteLine("A grab failure occurred. See the method ImageProvider::Grab for more information. The error code is {0:X08}.", grabResult.ErrorCode);
                    }
                }
        ...
        private async Task<uint> WaitObjectsWaitForAny(PYLON_WAITOBJECTS_HANDLE waitObjects, uint timeoutMs)
        {
            Task<uint> task = Task<uint>.Factory.StartNew(() =>
            {
                if (PylonC.NET.Pylon.WaitObjectsWaitForAny(waitObjects, timeoutMs, out uint _index))
                {
                    return _index;
                }
                else
        ...
        protected void EnqueueTakenImage(PylonC.NET.PylonGrabResult_t grabResult)
        {
            ...
            var newGrabResultInternal = new GrabResult();
            ...
            if (grabResult.PixelType == PylonC.NET.EPylonPixelType.PixelType_Mono8 || grabResult.PixelType == PylonC.NET.EPylonPixelType.PixelType_RGB8packed)
            {
                newGrabResultInternal.ImageData = new Image(grabResult.SizeX, grabResult.SizeY, buffer.Array, grabResult.PixelType == PylonC.NET.EPylonPixelType.PixelType_BGR8packed);
            }
            else /* Conversion is required. */
            {
                ...
                {
                    m_convertedBuffers = new Dictionary<PylonC.NET.PYLON_STREAMBUFFER_HANDLE, PylonC.NET.PylonBuffer<byte>>(); /* Create a new dictionary for the converted buffers. */
                ...
                PylonC.NET.Pylon.ImageFormatConverterSetOutputPixelFormat(m_hConverter, m_converterOutputFormatIsColor ? PylonC.NET.EPylonPixelType.PixelType_BGR8packed : PylonC.NET.EPylonPixelType.PixelType_Mono8);
                PylonC.NET.Pylon.ImageFormatConverterConvert(m_hConverter, ref convertedBuffer, buffer, grabResult.PixelType, (uint)grabResult.SizeX, (uint)grabResult.SizeY, (uint)grabResult.PaddingX, PylonC.NET.EPylonImageOrientation.ImageOrientation_TopDown);
        ...
        protected void CleanUpGrab()
        {
            ...
            /* ... The m_buffers can now be retrieved from the stream grabber. */
            {
                ...
                {
                    /* Stores the result of a grab operation. */
                    isReady = PylonC.NET.Pylon.StreamGrabberRetrieveResult(m_hGrabber, out PylonGrabResult_t grabResult);
        ...
        protected void RemovalCallbackHandler(PylonC.NET.PYLON_DEVICE_HANDLE hDevice)
        {
