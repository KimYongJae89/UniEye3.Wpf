...
namespace DynMvp.Vision
{
    public class PolyFit<T>
    {
        ...
        public PolyFit(T[] x, T[] y, int order)
        {
            ...
            {
                matrixY[i, 0] = Convert.ToDouble(y[i]);
            }
            ...
            for (int row = 0; row < x.Count(); row++)
            {
                ...
                for (int col = 0; col < order; col++)
                {
                    ...
                    nVal *= Convert.ToDouble(x[row]);
                }
            }

            double[,] matrixXt = matrixX.Transpose();
            double[,] matrixXtX = matrixXt.Product(matrixX);
            double[,] matrixXtY = matrixXt.Product(matrixY);
